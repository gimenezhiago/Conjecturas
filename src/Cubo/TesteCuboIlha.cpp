...
void torneio(const std::vector<Ind>&pop, int&a, int&b, std::mt19937&rng) {
    int sz = (int)pop.size();
    int c[4]; for(int i=0;i<4;i++) c[i] = rng() % sz;
...
int main(int argc, char **argv) {
    /* Uso: ./TesteCuboIlha <n_embaralha> <n_threads> [seed] */
    ...
    /* Inicialização das ilhas em paralelo */
    std::vector<Ilha> ilhas(NUM_ILHAS);
    tbb::parallel_for(0, NUM_ILHAS, [&](int id) {
        std::mt19937 rng(std::random_device{}() ^ ((uint32_t)id * 2654435761u));
        ilhas[id].pop.resize(TAM_ILHA);
        for(auto &ind : ilhas[id].pop) {
            ind.v.resize(n_embaralha);
            for(auto &m : ind.v) m = rng() % NUM_MOV;
            ind.f = fitness(cubo, ind.v);
        }
        std::sort(ilhas[id].pop.begin(), ilhas[id].pop.end(),
    ...
    for(int g = 1; !solucao; g++) {
        ...
        /* Atualiza g_conv se houve melhora em qualquer ilha nesta geração */
        {
            float bf_ant = 0;
            for(int id = 0; id < NUM_ILHAS; id++)
                if(ilhas[id].mg > bf_ant) bf_ant = ilhas[id].mg;
            /* mg das ilhas já foi atualizado dentro do parallel_for acima;
             * se alguma ilha melhorou (estag voltou a 0), registra a geração */
            for(int id = 0; id < NUM_ILHAS; id++)
                if(ilhas[id].estag == 0) { g_conv = g; break; }
        }
        ...
        if(g % INTERVALO_MIG == 0) {
            ...
            for(int id = 0; id < NUM_ILHAS; id++) {
                ...
                std::sort(ilhas[dest].pop.begin(), ilhas[dest].pop.end(),
                          [](const Ind&a, const Ind&b){ return a.f > b.f; });
            }
        }

        /* Para se todas as ilhas estagnaram */
        bool todas_estag = true;
        for(int id = 0; id < NUM_ILHAS; id++)
            if(ilhas[id].estag < MAX_ESTAG) { todas_estag = false; break; }
        if(todas_estag) break;
    }

    float bf = -1;
    for(int id = 0; id < NUM_ILHAS; id++)
        if(ilhas[id].pop[0].f > bf) bf = ilhas[id].pop[0].f;

    printf("RESULTADO,ModeloIlhas,%u,%d,%.4f,%s,%d\n",
           nthreads, n_embaralha, bf,
           solucao ? "SIM" : "NAO",
           g_conv);
