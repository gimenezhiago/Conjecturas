...
int main(int argc, char **argv){
    ...
    fprintf(stderr, "=== OPENMP — FITNESS PARALELO ===\n");
    fprintf(stderr, "Threads: %d | Pop: %d | Cromo: %d | Embaralha: %d | Seed: %u\n\n", nthreads, TAM_POP, n_embaralha, n_embaralha, seed);
    ...
    #pragma omp parallel
    {
        unsigned int local_seed = seed ^ (unsigned int)(omp_get_thread_num() * 0x9e3779b9);
    ...
    for(int g=1; estag < MAX_ESTAG; g++){
        ...
        #pragma omp parallel
        {
            unsigned int local_seed = seed ^ (unsigned int)(omp_get_thread_num() * 2246822519u + (unsigned)g * 374761393u);
            #pragma omp for schedule(static)
            for(int i=0;i<TAM_POP;i++){
                int a,b; torneio(pop, TAM_POP, &a, &b, &local_seed);
                filhos[i] = cruzar(&pop[a], &pop[b], &local_seed);
                if(((float)(xorshift32(&local_seed) % 10000) / 10000.f) < taxa) mutar(&filhos[i], &local_seed);
                filhos[i].f = fitness(&cubo, filhos[i].v, filhos[i].len);
            }
        }
        // merge and select best
        for(int i=0;i<TAM_POP;i++){ // append
            // free worst slot and replace by child (we'll push into array)
        }
        ...
        // free extra individual's v pointers (the ones beyond TAM_POP)
    ...
    printf("RESULTADO,FitnessParalelo,%d,%d,%.4f,%s,%d\n", nthreads, n_embaralha, pop[0].f, pop[0].f >= FIT_MAX ? "SIM" : "NAO", g_conv);
