...
__device__ bool ehQuadradoPrimo(long long int n) {
    long long raiz = (long long)sqrt((double)n);
    return (raiz * raiz == n && ehPrimo(raiz));
}

__device__ long long proximoPrimo(long long n) {
    long long p = n + 1;
    while (!ehPrimo(p)) {
        p++;
    }
...
__device__ long long aplicarRegrasTri(long long n) {
    if (n <= 1) return n;
    long long divisor = 2;
...
__device__ long long triangular(int m) {
...
__global__ void testeConvergenciaKernel(long long inicio, long long fim, int *resultados) {
    ...
    long long atual = triangular((int)numero) * triangular((int)numero + 1);
...
// Para compilar: nvcc -O3 TesteIntervaloTriParaleloCuda.cu -o TesteIntervaloTriParaleloCuda
// Para rodar: ./TesteIntervaloTriParaleloCuda 100
